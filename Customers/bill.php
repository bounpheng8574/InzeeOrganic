...
<html lang="EN">
	...
	<body onload="Print();" onclick="goBack();">
        ...
        <div class="pull-left"><img src="../photo/icon1.jpeg" style="width: 80px; height: 80px;"><br/>
        	ລະຫັດໃບບິນ: <?php
        	include("../config.php");
			$stmt = $DB_con->prepare("SELECT MAX(SellID) as SellID FROM sellproduct ");
			$stmt->execute();
			if($stmt->rowCount() > 0)
	{
	...
	<?php
		...
		<br />
		<div class="pull-left">&emsp; <b></b> <br/> <br/><br/>
		
		</div>		
		<div class="pull-right"><b>ລາຍລະອຽດຜູ້ສັ່ງຈອງ</b> <br/>
		<!-- <?php	
		$sql //=mysqli_query($dbcon, "SELECT * FROM payment where CustID='$CustID'");
               //  while ($row = mysqli_fetch_array($sql)) {?>   
        ຊື່ບັນຊີ:&nbsp; <?php //echo $row['accname']; ?> <br/>
		ໝາຍເລກບັນຊີ:&nbsp;<?php //echo $row['accno']; ?><br/>
		ບັນຊີທະນາຄານ:&nbsp;<?php //echo $row['accbank']; ?><br/>
		
<?php  } ?> -->
		<?php 
		echo $CustFirstName; 
		echo $CustLastName; ?><br/>
		<?php 
		echo $CustAddress; ?><br/> 
		<?php 
		echo $CustTel; ?> <br/>
